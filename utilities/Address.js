...
const aeRegex = `^(ak_[${aeAllowedChars}]{5})+([${aeAllowedChars}]{37})+([${aeAllowedChars}]{8})$`;
...
export const truncateAEAddress = (address) => {
  return truncateHandler(address, aeRegex, (match) => `${match[1]}…${match[3]}`);
};
...
export const truncateAddress = (address, token = "eth")=> {
