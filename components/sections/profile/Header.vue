...
export default {
  ...
  methods: {
    triggerDiscordOauth() {
      window.location.href = `${process.env.NUXT_ENV_DISCORD_OAUTH_BASE_URL}?client_id=${process.env.NUXT_ENV_DISCORD_CLIENT_ID}&redirect_uri=http://localhost:3000/profile&response_type=code&scope=${process.env.NUXT_ENV_DISCORD_SCOPE}`
    },
  },
}
</script>
