      ...
      <div class="col-lg-12">
        ...
        <div>
          <b-nav vertical class="w-25">
            <b-nav-item>
              <h2>
                <i @click="getSubmissions"> Total Submissions </i>
              </h2>
            </b-nav-item>
            <b-nav-item>
              <h2>
                <i @click="getReviews(getPreviousMonday)"> Total reviews </i>
              </h2>
            </b-nav-item>
            <b-nav-item>
              <h2>
          ...
          <div>
            ...
            <div v-if="totalReviews">Total Feedback: {{ totalReviews }}</div>
            <div>
              <div>
                <input
                  v-model="weekStartDate"
                  type="date"
                  placeholder="2019-11-04"
                />
...
export default {
  ...
  computed: {
    getPreviousMonday() {
      ...
      prevMonday.setDate(
        prevMonday.getDate() - ((prevMonday.getDay() + 6) % 14)
      )
      return prevMonday
  ...
  methods: {
    ...
    async getSubmissions() {
      ...
      for (let index = 0; index < communities.length; index++) {
        ...
        await firebase
          .database()
          .ref(`submissions/${communities[index]}`)
          .once('value', function (snapshot) {
            snapshot.forEach(function (childSnapshot) {
              ...
              if (element.data.submissionReward) {
                submissionRewards =
                  submissionRewards + parseFloat(element.data.submissionReward)
              }
              submissionsLength++
              communitySubmissionLength++
              communitySubmissions[communities[index]] =
                communitySubmissionLength
      ...
      await firebase
        ...
        .once('value', function (snapshot) {
          snapshot.forEach(function (childSnapshot) {
            ...
            for (
              let index = 0;
              index < Object.values(element.data).length;
              index++
            ) {
              ...
              if (Object.values(element.data)[index].date > 1510413525000) {
                if (Object.values(element.data)[index].rewardAmount) {
                  ...
                  rewards =
                    rewards +
                    parseFloat(Object.values(element.data)[index].rewardAmount)
                }
                rewardCount++
    ...
    async getReviewsForSubmission(elementF) {
      ...
      await firebase
        .database()
        .ref(`reviews/${elementF.submissionId}`)
        .once('value', function (snapshot) {
          snapshot.forEach(function (childSnapshot) {
            const element = {}
            element.key = childSnapshot.key
    ...
    async getSubmissionsSinceDate(dateRange) {
      ...
      for (let index = 0; index < submissionIds.length; index++) {
        ...
        if (newReviewsCount) {
          ...
          reviewsCount[element.community] =
            reviewsCount[element.community] + newReviewsCount
