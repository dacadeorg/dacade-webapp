        ...
        <div>
          <b-nav vertical class="w-25">
            <b-nav-item>
              <h2>
                <i>
                  <nuxt-link to="/admin/scoreboard/"> scoreboard </nuxt-link>
                </i>
              </h2>
            </b-nav-item>
            <b-nav-item>
              <h2>
                <i @click="getSubmissions"> getSubmissions </i>
              </h2>
            </b-nav-item>
            <b-nav-item>
              <h2>
                <i @click="getReviews"> getReviews </i>
              </h2>
            </b-nav-item>
            <b-nav-item>
              <h2>
                <i @click="getUsers"> getUsers </i>
              </h2>
            </b-nav-item>
            <b-nav-item>
              <h2>
                <i @click="getAllUserReviews"> getAllUserReviews </i>
              </h2>
            </b-nav-item>
          </b-nav>
        </div>
  ...
  methods: {
    ...
    async getAllUserReviews() {
      ...
      await firebase
        .database()
        .ref('reviews')
        .orderByChild('reviewUserId')
        .equalTo('zbhLf4OXIPMzA7E66zQnN0XDip02')
        .once('value', function (snapshot) {
          snapshot.forEach(function (childSnapshot) {
            const element = {}
            element.key = childSnapshot.key
