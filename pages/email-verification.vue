<template>
  <div
    v-if="user"
    class="md:w-7/12 w-full content-wrapper mx-auto"
  >
    <div
    ...
    >
      <div>
        <h1 class="text-3xl font-normal mb-3">
          ...
          <p class="text-lg">
            <span class="font-normal">{{ $t('email-verification.subtitle') }}</span>
            <span class="font-medium">{{ user.email }}</span>
            <span class="font-normal" v-html="$t('email-verification.message')"/>
          </p>
          <!-- <p class="text-base font-bold mb-4 inline-block"></p>
          <p
            class="text-lg py-4 inline-block"
            
          /> -->

        <p class="text-sm pt-7 pb-5">{{ $t('email-verification.foot-note') }}</p>
      </div>
