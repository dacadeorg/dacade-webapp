...
export const actions = {
  createPayoutRequest({ commit }, payload) {
    ...
    firebase
      .database()
      .ref(`payoutRequests/${communityId}/${userId}`)
      .set(payload)
      .then(() => {
  ...
  updatePayoutRequestAsPaid({ commit }, payload) {
    firebase
      .database()
      .ref(`payoutRequests/${payload.communityId}/${payload.userId}/paid`)
      .set(Date.now())
      .then(() => {
  ...
  setPayoutRequestPending({ commit }, payload) {
    firebase
      .database()
      .ref(`payoutRequestsPending/${payload.userId}/${payload.tokenFormat}`)
      .set(payload.payoutAmount)
      .then(() => {
